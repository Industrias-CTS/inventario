...
const createMovement = async (req, res) => {
    ...
    try {
        ...
        const { movement_type_id, component_id, quantity, unit_cost = 0, reference_number, notes, recipe_id } = req.body;
...
const createReservation = async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        const { component_id, quantity, reference, notes, expires_at } = req.body;
    ...
    `;
        ...
        const movementTypeResult = await client.query("SELECT id FROM movement_types WHERE code = 'RESERVATION'", []);
      ...
      VALUES ($1, $2, $3, $4, $5, $6)`, [movement_type_id, component_id, quantity, reference, notes, reserved_by]);
        // Actualizar stock reservado
        await client.query('UPDATE components SET reserved_stock = reserved_stock + $1 WHERE id = $2', [quantity, component_id]);
        await client.query('COMMIT');
        res.status(201).json({
