...
exports.deleteProjection = exports.updateProjection = exports.createProjection = exports.getProjectionById = exports.getProjections = void 0;
...
const getProjectionById = async (req, res) => {
    try {
       ...
       WHERE pr.projection_id = ?`, [id]);
        ...
        const requirements = await database_config_1.db.query(`SELECT preq.*, c.name as component_name, c.code as component_code, u.symbol as unit_symbol
       FROM projection_requirements preq
       JOIN components c ON preq.component_id = c.id
       LEFT JOIN units u ON c.unit_id = u.id
       WHERE preq.projection_id = ?`, [id]);
        res.json({
            projection: {
                ...projection,
                recipes,
...
const createProjection = async (req, res) => {
    try {
        ...
        await database_config_1.db.transaction(async () => {
            ...
            // Crear la proyección
            await database_config_1.db.run(`INSERT INTO projections (
          id, name, description, user_id, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?)`, [projectionId, name, description || '', userId, now, now]);
            // Insertar recetas
            for (const recipe of recipes) {
                await database_config_1.db.run(`INSERT INTO projection_recipes (
            projection_id, recipe_id, quantity
          ) VALUES (?, ?, ?)`, [projectionId, recipe.recipe_id, recipe.quantity]);
            }
         ...
         WHERE pr.projection_id = ?`, [projectionId]);
            const projectionRequirements = await database_config_1.db.query(`SELECT preq.*, c.name as component_name, c.code as component_code, u.symbol as unit_symbol
         FROM projection_requirements preq
         JOIN components c ON preq.component_id = c.id
         LEFT JOIN units u ON c.unit_id = u.id
         WHERE preq.projection_id = ?`, [projectionId]);
            res.status(201).json({
...
const updateProjection = async (req, res) => {
    try {
        ...
        const existingProjection = await database_config_1.db.get('SELECT * FROM projections WHERE id = ?', [id]);
        ...
        await database_config_1.db.transaction(async () => {
            ...
            await database_config_1.db.run(`UPDATE projections 
         SET name = ?, description = ?, updated_at = ?
         WHERE id = ?`, [name, description || '', now, id]);
            // Eliminar recetas existentes y agregar nuevas
            if (recipes && Array.isArray(recipes)) {
                await database_config_1.db.run('DELETE FROM projection_recipes WHERE projection_id = ?', [id]);
                for (const recipe of recipes) {
                    await database_config_1.db.run(`INSERT INTO projection_recipes (
              projection_id, recipe_id, quantity
            ) VALUES (?, ?, ?)`, [id, recipe.recipe_id, recipe.quantity]);
                }
            }
...
const deleteProjection = async (req, res) => {
    try {
        ...
        await database_config_1.db.transaction(async () => {
            ...
            await database_config_1.db.run('DELETE FROM projections WHERE id = ?', [id]);
...
exports.deleteProjection = deleteProjection;
