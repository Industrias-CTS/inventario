  ...
  `);
    ...
    db.all("PRAGMA table_info(movements)", (err, columns) => {
        ...
        if (!hasTotal) {
            db.run("ALTER TABLE movements ADD COLUMN total_cost REAL DEFAULT 0", (err) => {
                if (err)
                    console.error('Error agregando columna total_cost:', err);
                else
                    console.log('✅ Columna total_cost agregada a movements');
            });
        }
        if (!hasUserId) {
            db.run("ALTER TABLE movements ADD COLUMN user_id TEXT", (err) => {
                if (err)
                    console.error('Error agregando columna user_id:', err);
                else
                    console.log('✅ Columna user_id agregada a movements');
            });
        }
    });
    ...
    db.all("PRAGMA table_info(components)", (err, columns) => {
        ...
        if (!hasUserId) {
            db.run("ALTER TABLE components ADD COLUMN created_by TEXT", (err) => {
                if (err)
                    console.error('Error agregando columna created_by:', err);
                else
                    console.log('✅ Columna created_by agregada a components');
            });
        }
    });
    ...
    db.get("SELECT COUNT(*) as count FROM users", (err, row) => {
        ...
        if (row.count === 0) {
            ...
            db.run(`INSERT INTO components (id, code, name, description, category_id, unit_id, min_stock, current_stock, cost_price, sale_price) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [componentId, 'RES001', 'Resistencia 1K Ohm', 'Resistencia de carbón 1/4W', categoryId, unitId, 10, 50, 0.50, 1.00]);
            db.run(`INSERT INTO components (id, code, name, description, category_id, unit_id, min_stock, current_stock, cost_price, sale_price) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [generateId(), 'CAP001', 'Capacitor 100uF', 'Capacitor electrolítico', categoryId, unitId, 5, 25, 2.00, 4.00]);
            db.run(`INSERT INTO components (id, code, name, description, category_id, unit_id, min_stock, current_stock, cost_price, sale_price) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [generateId(), 'LED001', 'LED Rojo 5mm', 'LED de alta luminosidad', categoryId, unitId, 20, 100, 0.25, 0.75]);
    ...
    db.get("SELECT COUNT(*) as count FROM users WHERE username = 'admin'", async (err, row) => {
        ...
        if (row.count === 0) {
            ...
            db.run(`INSERT INTO users (id, username, email, password, first_name, last_name, role)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [adminId, 'admin', 'admin@inventory.com', hashedPassword, 'Super', 'Admin', 'admin'], function (err) {
                if (err) {
    ...
    db.get('SELECT * FROM users WHERE username = ? OR email = ?', [username, username], async (err, user) => {
        ...
        try {
            ...
            const token = jwt.sign({
                ...
                email: user.email,
                role: user.role,
            }, 'supersecretkey123456789inventory', { expiresIn: '7d' });
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        db.run(`INSERT INTO users (id, username, email, password, first_name, last_name, role)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [userId, username, email, hashedPassword, first_name, last_name, role], function (err) {
            if (err) {
...
app.post('/api/components', authenticate, (req, res) => {
    ...
    const componentId = generateId();
    db.run(`INSERT INTO components (id, code, name, description, category_id, unit_id, min_stock, max_stock, location, cost_price, sale_price, current_stock, reserved_stock, created_by)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 0, ?)`, [componentId, code, name, description, category_id || null, unit_id, min_stock || 0, max_stock || null, location || null, cost_price || 0, sale_price || 0, req.user.userId], function (err) {
        if (err) {
            if (err.message.includes('UNIQUE constraint failed')) {
        ...
        db.get(`SELECT c.*, cat.name as category_name, u.name as unit_name, u.symbol as unit_symbol
         FROM components c
         LEFT JOIN categories cat ON c.category_id = cat.id
         LEFT JOIN units u ON c.unit_id = u.id
         WHERE c.id = ?`, [componentId], (err, component) => {
            if (err) {
...
app.put('/api/components/:id', authenticate, (req, res) => {
    ...
    const { id } = req.params;
    const { code, name, description, category_id, unit_id, min_stock, max_stock, location, cost_price, sale_price } = req.body;
    db.run(`UPDATE components SET code = ?, name = ?, description = ?, category_id = ?, unit_id = ?, 
     min_stock = ?, max_stock = ?, location = ?, cost_price = ?, sale_price = ?, updated_at = CURRENT_TIMESTAMP
     WHERE id = ?`, [code, name, description, category_id || null, unit_id, min_stock || 0, max_stock || null, location || null, cost_price || 0, sale_price || 0, id], function (err) {
        if (err) {
            if (err.message.includes('UNIQUE constraint failed')) {
                return res.status(400).json({ error: 'El código del componente ya existe' });
            }
...
app.post('/api/movements', authenticate, (req, res) => {
    ...
    db.get('SELECT * FROM movement_types WHERE id = ?', [movement_type_id], (err, movementType) => {
        ...
        function insertMovement() {
            ...
            db.run(`INSERT INTO movements (id, movement_type_id, component_id, quantity, unit_cost, total_cost, reference_number, notes, user_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [movementId, movement_type_id, component_id, quantity, unit_cost || 0, total_cost, reference_number || null, notes || null, req.user.userId], function (err) {
                if (err) {
          ...
          WHERE m.id = ?`, [movementId], (err, movement) => {
                    if (err) {
...
app.post('/api/movements/reservations', authenticate, (req, res) => {
    ...
    db.get('SELECT current_stock, reserved_stock FROM components WHERE id = ?', [component_id], (err, component) => {
        ...
        // Crear la reserva
        db.run(`INSERT INTO reservations (id, component_id, quantity, reference, expires_at, notes, reserved_by)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [reservationId, component_id, quantity, reference || null, expires_at || null, notes || null, req.user.userId], function (err) {
            if (err) {
            ...
            JOIN components c ON r.component_id = c.id
            JOIN users u ON r.reserved_by = u.id
            WHERE r.id = ?`, [reservationId], (err, reservation) => {
                    if (err) {
...
app.post('/api/units', authenticate, (req, res) => {
    ...
    const unitId = generateId();
    db.run('INSERT INTO units (id, name, symbol) VALUES (?, ?, ?)', [unitId, name, symbol], function (err) {
        if (err) {
            if (err.message.includes('UNIQUE constraint failed')) {
...
app.put('/api/units/:id', authenticate, (req, res) => {
    ...
    const { id } = req.params;
    const { name, symbol } = req.body;
    db.run('UPDATE units SET name = ?, symbol = ? WHERE id = ?', [name, symbol, id], function (err) {
        if (err) {
            if (err.message.includes('UNIQUE constraint failed')) {
                return res.status(400).json({ error: 'El nombre o símbolo de la unidad ya existe' });
            }
...
app.post('/api/movement-types', authenticate, (req, res) => {
    ...
    db.run('INSERT INTO movement_types (id, code, name, operation) VALUES (?, ?, ?, ?)', [movementTypeId, code, name, operation], function (err) {
        if (err) {
            if (err.message.includes('UNIQUE constraint failed')) {
...
app.post('/api/categories', authenticate, (req, res) => {
    ...
    const categoryId = generateId();
    db.run('INSERT INTO categories (id, name, description) VALUES (?, ?, ?)', [categoryId, name, description], function (err) {
        if (err) {
            if (err.message.includes('UNIQUE constraint failed')) {
...
//# sourceMappingURL=index-simple-db.js.map